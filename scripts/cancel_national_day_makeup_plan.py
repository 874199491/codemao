#!/usr/bin/env python3
"""Cancel National Day makeup-plan WeCom pending sends."""
...
RUNTIME = WORKSPACE / "data" / "国庆补课计划"
RESULTS = RUNTIME / "send-results"
...
def target_ids(student_ids: list[str]) -> list[str]:
    ...
    for path in sorted(RESULTS.glob("*.json")):
        try:
            ...
            continue
        if isinstance(payload, dict) and payload.get("created") is True:
            ids.append(str(payload.get("student_id") or path.stem).strip())
...
def mark_sendable(ids: list[str], reason: str) -> dict[str, Any]:
    ...
    for student_id in ids:
        result_path = RESULTS / f"{student_id}.json"
    ...
    return {"changed_result_files": changed_results}
...
def main() -> int:
    ...
    ids = target_ids(args.student_id)
    print(json.dumps({"mode": "execute" if args.execute else "dry-run", "target_count": len(ids), "student_ids": ids}, ensure_ascii=False, indent=2), flush=True)
    if not ids:
        return 0
    sess = session()
    ...
    print(json.dumps({"cancel_record_count": len(targets), "matched_student_count": len(matched_ids), "missing_student_count": len(missing_ids), "missing_student_ids": missing_ids}, ensure_ascii=False, indent=2), flush=True)
    if not args.execute:
        return 0

    canceled, failed, already_confirmed, canceled_student_ids = cancel_targets(sess, targets)
    local = mark_sendable(canceled_student_ids, "取消国庆补课群发后恢复可发送")
    summary = {
