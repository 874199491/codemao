...
def render_card(template: Path, name: str, lessons: list[str], out: Path) -> Path:
    ...
    if len(visible_lessons) < len(lesson_slots):
        visible_lessons.extend(["好好休息"] * (len(lesson_slots) - len(visible_lessons)))
    for idx, rect in enumerate(lesson_slots):
        text = visible_lessons[idx]
        text_cover = sc_rect((rect[0] - 8, rect[1] - 20, 1028, rect[3] + 18))
        draw.rounded_rectangle(text_cover, radius=round(8 * sx), fill=row_fill)
